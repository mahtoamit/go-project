...
// put tablename and Qtype in RedisSetNewCache
func RedisSetNewCache(books models.Book, key string) (flag1 bool, err error) {
	// Store the data in the Redis cache for future use

	// two field added in redis table_name,Qtype
	tablename := "books"
	qtype := "insert"
	updatedDate := models.CacheModel{
		DatabaseName: tablename,
		QueryType:    qtype,
		Model:        books,
	}
	data, err := json.Marshal(updatedDate)
...
func RetrieveData() (string, error) {
	...
	cachedData, err := redisClient.LPop(ctx, "booksData").Result()
